...
export default class DashboardLayout {
    ...
    async initialize() {
        ...
        try {
            this.initGrid(); // 1. Inicializar GridStack primero
            await this.loadAndRenderWidgets(); // 2. Cargar layout y renderizar widgets
            console.log('[Layout] Layout inicializado correctamente.');
        } catch (error) {
            console.error('[Layout] Error al inicializar el layout:', error);
    ...
    initGrid() {
        console.log('[Layout] Inicializando GridStack...');
        ...
        console.log('[Layout] GridStack inicializado y listeners configurados.');
    }
    ...
    async loadAndRenderWidgets() {
        console.log("[Layout] Buscando layout guardado en localStorage...");
    ...
    addWidget(widgetId, layoutOptions = {}) {
        ...
        const addedWidgetNode = this.grid.addWidget(gridOptions);
        console.log(`[Layout] Widget '${widgetId}' añadido al grid.`);

        // Inicializar el manager del widget usando setTimeout para asegurar que su
        // contenido interno esté completamente renderizado en el DOM.
        if (widgetInfo.manager?.initializeWidget) {
            console.log(`[Layout] Programando inicialización del manager para el widget '${widgetId}'.`);
            setTimeout(() => {
                // Verificamos que el nodo del widget todavía existe en el cuerpo del documento.
                // Esto previene errores si el widget es eliminado rápidamente.
                if (document.body.contains(addedWidgetNode)) {
                    widgetInfo.manager.initializeWidget(addedWidgetNode);
                } else {
                    console.warn(`[Layout] El widget '${widgetId}' fue eliminado antes de que su manager pudiera inicializarse.`);
                }
            }, 50);
        }
