...
export default class UIManager {
    ...
    renderTable(tableId, data, columns, extraOptions = {}) {
        ...
        if (!columns || columns.length === 0) {
            console.warn(`[UI] Se intentó renderizar la tabla '${tableId}' sin columnas definidas. Abortando.`);
            // Opcional: mostrar un mensaje de error en la tabla
            const tableElement = document.getElementById(tableId);
            if (tableElement) {
                tableElement.innerHTML = '<tr><td colspan="100%">Error: Configuración de columnas no encontrada.</td></tr>';
            }
            return;
        }
