...
from src.services.bolsa_service import run_bolsa_bot, is_bot_running as is_async_bot_running
from src.services import dividend_service, closing_service, ai_financial_service
from src.services.bot_page_manager import get_page_manager_instance
...
from src.tasks import run_bolsa_bot_task
...
@bot_bp.route("/dividends/update", methods=["POST"])
...
@bot_bp.route("/closing/update", methods=["POST"])
...
@bot_bp.route("/kpis/update", methods=["POST"])
...
def run_bot():
    """
    Endpoint para iniciar la tarea de scraping del bot de forma asíncrona.
    """
    try:
        # Encolar la tarea en Celery. '.delay()' es un atajo para '.apply_async()'.
        task = run_bolsa_bot_task.delay()
        
        # Devolver una respuesta inmediata al cliente con el ID de la tarea.
        return jsonify({
            "message": "La tarea de actualización del bot ha sido iniciada.",
            "task_id": task.id
        }), 202  # 202 Accepted
        
    except Exception as e:
        current_app.logger.error(f"Error al encolar la tarea del bot: {e}", exc_info=True)
        return jsonify({
            "error": "Error Interno del Servidor",
            "message": "No se pudo iniciar la tarea del bot."
        }), 500